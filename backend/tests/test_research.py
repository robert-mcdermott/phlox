...
def test_document_scope_blocks_web_exec_children_and_mcp(db, monkeypatch):
    ...
    assert all(set(p["tools"]) <= {"search_documents", "update_research_notebook"} for p in provider.seen)
...
def test_approval_preserves_research_counters_and_scope(db, monkeypatch):
    ...
    assert resumed.allowed_tools == {"web_search", "web_fetch", "read_web_source", "query_public_api", "export_api_dataset", "update_research_notebook"}
